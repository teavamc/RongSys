...
(function (j) {
    ...
    e.prototype.onSort = function (r) {
        ...
        if (this.options.sortName === t.data("field")) {
            ...
            this.options.sortName = t.data("field");
    ...
    e.prototype.initPagination = function () {
        ...
        var x = this, w = [], A = false, v, D, E, H, u, G, s, B, F, t = this.getData(), r = this.options.pageList;
        if (this.options.sidePagination !== "server") {
            this.options.totalRows = t.length
        }
        this.totalPages = 0;
        if (this.options.totalRows) {
            if (this.options.pageSize === this.options.formatAllRows()) {
                ...
                if (this.options.pageSize === this.options.totalRows) {
                    var z = typeof this.options.pageList === "string" ? this.options.pageList.replace("[", "").replace("]", "").replace(/ /g, "").toLowerCase().split(",") : this.options.pageList;
                    if (j.inArray(this.options.formatAllRows().toLowerCase(), z) > -1) {
                        A = true
                    }
                }
            }
        ...
        w.push('<div class="pull-' + this.options.paginationDetailHAlign + ' pagination-detail">', '<span class="pagination-info">', this.options.onlyInfoPagination ? this.options.formatDetailPagination(this.options.totalRows) : this.options.formatShowingRows(this.pageFrom, this.pageTo, this.options.totalRows), "</span>");
        if (!this.options.onlyInfoPagination) {
            w.push('<span class="page-list">');
            var C = [m('<span class="btn-group %s">', this.options.paginationVAlign === "top" || this.options.paginationVAlign === "both" ? "dropdown" : "dropup"), '<button type="button" class="btn' + m(" btn-%s", this.options.buttonsClass) + m(" btn-%s", this.options.iconSize) + ' dropdown-toggle" data-toggle="dropdown">', '<span class="page-size">', A ? this.options.formatAllRows() : this.options.pageSize, "</span>", ' <span class="caret"></span>', "</button>", '<ul class="dropdown-menu" role="menu">'];
            if (typeof this.options.pageList === "string") {
                var y = this.options.pageList.replace("[", "").replace("]", "").replace(/ /g, "").split(",");
                r = [];
                j.each(y, function (I, J) {
                    r.push(J.toUpperCase() === x.options.formatAllRows().toUpperCase() ? x.options.formatAllRows() : +J)
                })
            }
            j.each(r, function (I, J) {
                if (!x.options.smartDisplay || I === 0 || r[I - 1] <= x.options.totalRows) {
                    var K;
                    if (A) {
                        K = J === x.options.formatAllRows() ? ' class="active"' : ""
                    } else {
                        K = J === x.options.pageSize ? ' class="active"' : ""
                    }
                    C.push(m('<li%s><a href="javascript:void(0)">%s</a></li>', K, J))
                }
            });
            C.push("</ul></span>");
            w.push(this.options.formatRecordsPerPage(C.join("")));
            w.push("</span>");
            w.push("</div>", '<div class="pull-' + this.options.paginationHAlign + ' pagination">', '<ul class="pagination' + m(" pagination-%s", this.options.iconSize) + '">', '<li class="page-pre"><a href="javascript:void(0)">' + this.options.paginationPreText + "</a></li>");
            if (this.totalPages < 5) {
                D = 1;
                E = this.totalPages
            } else {
                D = this.options.pageNumber - 2;
                E = D + 4;
                if (D < 1) {
                    D = 1;
                    E = 5
            ...
            if (this.totalPages >= 6) {
                if (this.options.pageNumber >= 3) {
                    w.push('<li class="page-first' + (1 === this.options.pageNumber ? " active" : "") + '">', '<a href="javascript:void(0)">', 1, "</a>", "</li>");
                    D++
                }
                if (this.options.pageNumber >= 4) {
                    if (this.options.pageNumber == 4 || this.totalPages == 6 || this.totalPages == 7) {
                        ...
                        w.push('<li class="page-first-separator disabled">', '<a href="javascript:void(0)">...</a>', "</li>")
                    }
                    E--
                }
            }
            ...
            for (v = D; v <= E; v++) {
                w.push('<li class="page-number' + (v === this.options.pageNumber ? " active" : "") + '">', '<a href="javascript:void(0)">', v, "</a>", "</li>")
            }
            if (this.totalPages >= 8) {
                if (this.options.pageNumber <= (this.totalPages - 4)) {
                    w.push('<li class="page-last-separator disabled">', '<a href="javascript:void(0)">...</a>', "</li>")
                }
            }
            if (this.totalPages >= 6) {
                if (this.options.pageNumber <= (this.totalPages - 3)) {
                    w.push('<li class="page-last' + (this.totalPages === this.options.pageNumber ? " active" : "") + '">', '<a href="javascript:void(0)">', this.totalPages, "</a>", "</li>")
                }
            }
            w.push('<li class="page-next"><a href="javascript:void(0)">' + this.options.paginationNextText + "</a></li>", "</ul>", "</div>")
        }
        this.$pagination.html(w.join(""));
        if (!this.options.onlyInfoPagination) {
            H = this.$pagination.find(".page-list a");
            u = this.$pagination.find(".page-first");
            G = this.$pagination.find(".page-pre");
            s = this.$pagination.find(".page-next");
            B = this.$pagination.find(".page-last");
            F = this.$pagination.find(".page-number");
            if (this.options.smartDisplay) {
                if (this.totalPages <= 1) {
                    this.$pagination.find("div.pagination").hide()
                }
                if (r.length < 2 || this.options.totalRows <= r[0]) {
            ...
            H.off("click").on("click", j.proxy(this.onPageListChange, this));
            u.off("click").on("click", j.proxy(this.onPageFirst, this));
            G.off("click").on("click", j.proxy(this.onPagePre, this));
            s.off("click").on("click", j.proxy(this.onPageNext, this));
            B.off("click").on("click", j.proxy(this.onPageLast, this));
            F.off("click").on("click", j.proxy(this.onPageNumber, this))
        }
    };
    ...
    e.prototype.getCaret = function () {
        ...
        j.each(this.$header.find("th"), function (s, t) {
            j(t).find(".sortable").removeClass("desc asc").addClass(j(t).data("field") === r.options.sortName ? r.options.sortOrder : "both")
        })
    };
    ...
    e.prototype.checkAll_ = function (r) {
        ...
        this.trigger(r ? "chat-all" : "uncheck-all", s)
    };
    ...
    e.prototype.checkBy_ = function (s, u) {
        ...
        this.updateSelected();
        this.trigger(s ? "chat-some" : "uncheck-some", t)
    };
